...
const Event = ({ item, filterDate }: IEventProps) => {
  ...
  const eventDuration = timeConversion(
    +new Date(item.to) - +new Date(item.from)
  );

  return (
    <div
      className={classModifier("event", [
    ...
    >
      {item.type === EventTypes.rest ? (
        ...
        <>
          <div className="event__avatar">
            <LazyLoadImage
              src={getContactAvatar(item.agency || { img: null, type: 2 })}
            />
          </div>

          <div className="event__info">
            {item.agency && (
              <div className="event__name">{item.agency.name}</div>
            )}

            <div className="event__time">
              {getTimeRange(item.from, item.to)}
            </div>
          </div>
        </>
      )}
      ...
      <span className="event__duration">{eventDuration}</span>
    </div>
  );
};
