...
{
    public function emailRegistration(EmailRegistrationRequest $request, IAuth $auth)
    {
        return response()->json($auth->registerWithEmail($request), Response::HTTP_CREATED);
    }

    public function validateEmailToken(ValidateEmailTokenRequest $request, IAuth $auth)
    {
        return response()->json($auth->validateRegistrationToken($request));
    }

    public function registration(RegistrationRequest $request, IAuth $auth)
    {
        return response()->json($auth->registrationConcreteUser($request), Response::HTTP_CREATED);
    }

    public function login(LoginRequest $request, IAuth $auth)
    {
        return response()->json($auth->login($request));
    }

    public function logout(Request $request, IAuth $auth)
    {
